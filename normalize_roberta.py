...
SIDS = [sid.strip() for sid in open(
    HOME + 'data/psytar/labels.txt', 'r').readlines()]
...
def train_model_get_pred(tr_txt, tr_lab, te_txt, te_lab, lbl_ix):
    ...
    for _epoch in tqdm(range(MAX_EPOCHS)):
        ...
        # print('The test accuracy for {} epoch is {}'.format(epoch, acc))
    pprint(max(global_acc))
    return te_prd


def main(dir_name, n_folds):
    """The main function"""
...
if __name__ == '__main__':
    data_path = 'data/psytar/'
    num_folds = 5
    main(data_path, num_folds)
